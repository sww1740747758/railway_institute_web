    ...
    if (active.value < 5) {
        if (active.value == 2) {
            ...
            ruleFormRef3.value.validate().then(valid => {
                if (valid) {
                    const returnedTarget = Object.assign(ruleFormRef.value.ruleForm, ruleFormRef2.value.ruleForm, ruleFormRef3.value.ruleForm);
                    registerMember(returnedTarget, radio1.value).then(() => {
